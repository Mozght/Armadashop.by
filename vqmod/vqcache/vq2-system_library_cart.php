		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$option_points = 0;
					$option_weight = 0;
// BOF - Zappo - Group Discounts - TWO LINES - Get Discount
					$subtotal = $this->checkSubTotal();
					$discount = $this->customer->getDiscount($product_id, $subtotal);
...
// BOF - Zappo - Group Discounts - TWO LINES - Get Options Discount
							$odiscount = $this->customer->getOptionDiscount($option_query->row['option_id'], $subtotal);
							if ($odiscount === false) $odiscount = $discount;
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
								...
								if ($option_value_query->num_rows) {
									if ($option_value_query->row['price_prefix'] == '+') {
										$option_price += ($option_value_query->row['price'] * $odiscount);
...
// BOF - Zappo - Group Discounts - Get Total Price in Cart (Excluding Discounts)
						...
						if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
							...
							if ($option_value_query->num_rows) {
								...
								foreach ($option_value as $product_option_value_id) {
									...
									if ($option_value_query->num_rows) {
										if ($option_value_query->row['price_prefix'] == '+') {
											$price += ($option_value_query->row['price'] * $odiscount);
										} elseif ($option_value_query->row['price_prefix'] == '-') {
											$price -= ($option_value_query->row['price'] * $odiscount);
										}
									}
...
// EOF - Zappo - Group Discounts - Get Total Price in Cart (Excluding Discounts)
}
?>