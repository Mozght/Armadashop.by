...
// BOF - Zappo - Group Discounts - Added Database column Discounts
		$discounts = '';
		foreach ($data['discounts'] as $gId => $dc) $discounts .= ' '.$gId.'|'.$dc['discount'].'@'.$dc['discount_minimum'];
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', discounts = '" . trim($discounts) . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
...
// BOF - Zappo - Group Discounts - ONE LINE - Clear Product cache
...
// BOF - Zappo - Group Discounts - Added Database column Discounts
		$discounts = '';
		foreach ($data['discounts'] as $gId => $dc) $discounts .= ' '.$gId.'|'.$dc['discount'].'@'.$dc['discount_minimum'];
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', discounts = '" . trim($discounts) . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
