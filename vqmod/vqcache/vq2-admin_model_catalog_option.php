...
// BOF - Zappo - Group Discounts - TWO LINES - "implode" Discounts into string
		$discounts = '';
		foreach ($data['discounts'] as $gId => $dc) $discounts .= ' '.$gId.'|'.$dc['discount'].'@'.$dc['discount_minimum'];
		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "', discounts = '" . trim($discounts) . "'");
...
// BOF - Zappo - Group Discounts - TWO LINES - "implode" Discounts into string
		$discounts = '';
		foreach ($data['discounts'] as $gId => $dc) $discounts .= ' '.$gId.'|'.$dc['discount'].'@'.$dc['discount_minimum'];
		$this->db->query("UPDATE `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "', discounts = '" . trim($discounts) . "' WHERE option_id = '" . (int)$option_id . "'");
